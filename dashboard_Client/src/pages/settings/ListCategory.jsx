...
export default function ListCategory() {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = LIST.map((n) => n.category);
 ...
 const epoch = Math.round(Date.now() / 1000);
  const handleDelete =async(rowId)=>{
    try{
      ...
      const res = await window.api.deleteCategory({id:rowId});
      if(res){
        ...
        <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
         ...
         {/* <Button variant="contained" onClick={()=>setOpen(!open)} startIcon={<Iconify icon="eva:funnel-fill" />}>
          ...
          <ButtonGroup variant='contained'>
            ...
            <Button onClick={()=>navigate('/dashboard/settings/add-category')}>Add Category</Button>
                    ...
                    return (
                      ...
                      >
                        <TableCell padding="checkbox">
                          <Checkbox checked={isItemSelected} onChange={(event) => handleClick(event, category)} />
                        </TableCell>
